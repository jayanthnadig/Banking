    ...
    switch(action.type){
        ...
        case actionTypes.POST_DASHBOARD_WIDGET:
            ...
            if(_newwidget.data.length){
              _dashboard.widgetData=JSON.parse(JSON.stringify( _dashboard.widgetData));
              if(_dashboard.widgetData.filter(xx=>xx.widgetId==_newwidget.data[0].widgetId).length){
                var _index=-1;
                  _dashboard.widgetData.map((yy,jj)=>{
                    if(yy.widgetId==_newwidget.data[0].widgetId)
                     _index = jj;
                  });
                  if(_index!==-1)
                  _dashboard.widgetData[_index]=_newwidget.data[0];
              }
              else
              _dashboard.widgetData.push(_newwidget.data[0]);
           }
            state=_dashboard;
            return state;
        ...
        case actionTypes.DELETE_DASHBOARD_WIDGET:
              ...
              if(_deletewidget.code==200){
                _delete_dashboard.widgetData=JSON.parse(JSON.stringify(_delete_dashboard.widgetData));
                var _index=-1;
                _delete_dashboard.widgetData.map((yy,jj)=>{
                    if(yy.widgetId==_deletewidget.widgetId)
                     _index = jj;
                  });
                  if(_index!==-1)
                  _delete_dashboard.widgetData.splice(_index,1);
              ...
              case actionTypes.DRILLDOWN_DASHBOARD_WIDGET:
                ...
                if(_drilldowndata.code==200){
                  _drilldown_dashboard.drilldownData=JSON.parse(JSON.stringify(_drilldown_dashboard.drilldownData));
